...
**TypeScript project references** — each module under `src/` has its own `tsconfig.json` extending `tsconfig.base.json`. Build with `tsc --build` (via `yarn build`), not `tsc` directly. The root `tsconfig.json` lists all 10 modules as project references.
...
- `filter()` does not narrow types — it preserves the input element type `A` even with a type guard predicate.
...
- The `PrevIdx` tuple in `pipe` supports up to 21 transducers. Beyond that, type checking silently stops enforcing constraints.
